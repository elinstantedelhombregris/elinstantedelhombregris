...
ASSET_SLUG = "detectar-patrones-instagram-pilot-v3"
...
def make_frame(t: float, duration: float, captions: list[Caption], timings: list[WordTiming], ranges: dict[str, tuple[float, float]]) -> np.ndarray:
    ...
    draw_ascii_field(draw, field_for_scene(scene, t, progress), scene, t)
    draw_scanline(draw, t, scene)
    draw_top_meta(draw, scene, t)
    draw_registration_marks(draw, scene)
    draw_chapter_progress(draw, scene)

    scene_layer = Image.new("RGBA", (OUT_W, OUT_H), (0, 0, 0, 0))
    scene_draw = ScaledDraw(scene_layer)
    if scene == "noise":
        draw_noise_hook(scene_draw, progress)
    elif scene == "signal":
        draw_signal(scene_draw, progress, t)
    elif scene == "overload":
        draw_overload(scene_draw, progress, t)
    elif scene == "loop":
        draw_loop(scene_draw, progress, t)
    elif scene == "mirror":
        draw_mirror(scene_draw, progress, t)
    elif scene == "questions":
        draw_questions(scene_draw, progress)
    elif scene == "break":
        draw_break(scene_draw, progress, t)
    else:
        draw_final(scene_draw, progress)
...
def mix_audio(voice_wav: Path, mixed_wav: Path, music_wav: Path) -> tuple[float, int]:
    voice, sr = read_wav(voice_wav)
    music = generate_music(len(voice) / sr)[: len(voice)]
    ...
    mixed = normalized_voice + music * duck[:, None] * 0.26
...
def main() -> None:
    ...
    duration, _sr = mix_audio(voice_wav, mixed_wav, music_wav)
    ...
    manifest = {
        ...
        "creativeDirection": "noise-to-signal / political-loop / self-mirror / break-the-pattern",
