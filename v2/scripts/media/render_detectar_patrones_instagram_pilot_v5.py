...
ASSET_SLUG = "detectar-patrones-instagram-pilot-v5"
...
def draw_transition_overlay(draw: ScaledDraw, scene: str, blend: float) -> None:
    ...
    width = 4 + pulse * 7
    draw.rectangle((x - width, 112, x + width, 1118), fill=(*accent, int(3 + pulse * 5)))
    draw.line((x, 112, x, 1118), fill=(*accent, int(78 + pulse * 74)), width=1)
    draw.line((x - 3, 112, x - 3, 1118), fill=(*secondary, int(10 + pulse * 18)), width=1)
    draw.line((x + 3, 112, x + 3, 1118), fill=(*secondary, int(10 + pulse * 18)), width=1)
    for y in range(154, 1082, 86):
        tick = 12 + pulse * 18
        draw.line((x - tick, y, x + tick, y), fill=(*accent, int(22 + pulse * 38)), width=1)
...
def apply_cinematic_post(frame: np.ndarray, scene: str, frame_index: int, transition_blend: float) -> np.ndarray:
    ...
    if transition_blend < 1.0:
        ...
        wipe = np.exp(-((POST_X - transition_blend) / 0.035) ** 2)[:, :, None]
        tint = np.array(accent, dtype=np.float32)[None, None, :]
        result += wipe * tint * (0.035 + pulse * 0.075)
