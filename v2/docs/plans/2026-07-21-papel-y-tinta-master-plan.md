...
**Interfaces:**
- Produces: `GET /api/analytics/voces-count` → wire body `{ data: { total: number } }` per the app-wide envelope that `lib/api.ts` unwraps (ratified post-review; client-visible contract via `api.get` is `{ total }`). Counts APPROVED dreams only (`countApproved()` on the dreams repository — public number matches public data). Header shows `{N} voces · falta la tuya` formatted `es-AR` cuando `n ≥ 1`; con cero, cargando o error el slot dice «Falta la tuya.» a secas — sin constante de respaldo (régimen en `components/papel/voces-regimen.ts`, B10 de ①). Home `CifrasStrip` reuses the same `useVocesCount()` query in Phase 2.

- [ ] **Step 1:** Integration test: seed 3 dreams → endpoint returns `{ total: 3 }`. **Step 2:** FAIL. **Step 3:** implement repository `countAll()` + route. **Step 4:** PASS. **Step 5:** wire header query (sin constante de respaldo: régimen «Falta la tuya.» en `components/papel/voces-regimen.ts`, B10 de ①). **Step 6: Commit** — `feat(api,web): live voces counter behind the FOMO tagline`
...
| 2.4 | **La prueba** | `/planes` + `/planes/:slug` | LOS 22 PLANES | Callout «No es doctrina» · expandable plan rows (`FilaIndiceExpandible`) · method cards. NO filter chips (ratified 2026-07-22: planes MDX has no `categoria:` frontmatter and v1's taxonomy doesn't map — chips would be invented taxonomy; spec includes the revival recipe if the author adds the field) · no search (23 fixed items). `PlanDetail` = papel-sobre-oscuro expediente with permanent sello EJEMPLO + the system's FIRST §10.8 print edition (pattern reused by Phase 3 lectores). Content: `content/planes/` (22 + PLANRUTA as `00`). |
