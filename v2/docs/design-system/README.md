...
- `#5227CC` violeta = la marca, lo accionable, "sueño". En oscuro usar `#9D85E8`.
...
## 9. Reglas de implementación (Design Components)

- Todo estilo inline en el template; `<helmet>` solo para fonts, `@keyframes` y resets. Sin clases CSS salvo las responsive `.v2-*` ya definidas.
- Hex literales, nunca `var(--token)`.
- Repetir la receta literal antes que abstraer.
- Móvil: colapsar grillas a 1 columna, padding 20px, ocultar columnas secundarias; targets táctiles ≥44px.
