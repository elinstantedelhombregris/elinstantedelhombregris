...
`Inicio → La idea (3 capítulos, despertar gris→color) → El mapa (soltar voz) → El mandato (oscura + documento papel) → La prueba (22 planes, filtros) → La biblioteca (manifiesto + 21 ensayos en 3 ciclos + 6 entrenamientos curados (catálogo completo detrás de «ver todos» — el número lo dice el registry, nunca el texto) + bitácora) → lectores (ensayo/curso/crónica/manifiesto) → la crónica del país que viene (novela especulativa en una sola ruta, /cronica — no confundir con las crónicas de la bitácora) → Sembrar (3 pasos → certificado semilla)`
