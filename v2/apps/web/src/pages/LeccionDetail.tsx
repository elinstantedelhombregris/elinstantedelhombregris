  ...
  useEffect(() => {
    ...
    cargarLeccion(cursoSlug, leccionSlug)
      .then((crudo) => {
        if (!vivo) return;
        setEstado(
          crudo === null ? { fase: 'error' } : { fase: 'listo', cuerpo: sinTituloDuplicado(crudo, titulo) },
        );
      })
      .catch(() => {
...
const CLASE_ANTERIOR = 'font-space text-tinta-50 hover:text-tinta max-w-[300px] text-xs uppercase tracking-[0.06em]';
  ...
  return (
    <main className="mx-auto max-w-[800px] px-10 pb-20 pt-12 max-[560px]:px-5 print:p-0">
      ...
      <nav className="border-tinta mt-11 flex flex-wrap justify-between gap-5 border-t pt-[22px] print:hidden">
        {anterior ? (
          <Link href={`/entrenamientos/${cursoSlug}/leccion/${anterior.posicion}`} className={CLASE_ANTERIOR}>
            ← {anterior.leccion.titulo}
          </Link>
        ) : null}
        {siguiente ? (
          <Link href={`/entrenamientos/${cursoSlug}/leccion/${siguiente.posicion}`} className={CLASE_SIGUIENTE}>
