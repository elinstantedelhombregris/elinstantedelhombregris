import type { GeoPoint, LocationPrecision, PublicLocation } from './types.js';

const METERS_PER_DEGREE_LAT = 111_320;

/**
 * El lado aproximado de la celda pública de cada precisión, en metros.
 * `exact` devuelve null: no hay celda, el punto es el punto.
 */
const precisionMeters = (precision: LocationPrecision): number | null => {
  ...
  if (precision === 'city') return 5_000;
  // 'province' — el agregado de v2. No se dibuja como punto con halo sino
  // como lavado sobre la provincia entera, pero necesita un número para que
  // el resto de las funciones no tengan que tratarlo como caso especial.
  return 150_000;
};
 ...
 *
 * Es el radio del halo del render honesto (spec 1 §5): el halo ES el dato.
 * `exact` devuelve 0 — punto nítido, sin halo, porque no hay incertidumbre
 * que dibujar.
 */
